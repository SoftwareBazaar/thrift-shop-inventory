...
const OfflineIndicator: React.FC = () => {
  ...
  if (status.pendingOperations > 0) {
    return (
      <div className="fixed bottom-4 right-4 bg-orange-500 text-white px-4 py-2 rounded-lg shadow-lg flex items-center gap-2 z-50 cursor-pointer hover:bg-orange-600 transition-colors" onClick={handleManualSync}>
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className="fixed bottom-4 right-4 bg-green-500 text-white px-4 py-2 rounded-lg shadow-lg flex items-center gap-2 z-50 opacity-75 hover:opacity-100 transition-opacity cursor-pointer" onClick={handleManualSync} title="Last synced: {formatLastSync(status.lastSyncTime)}">
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
      </svg>
      <span className="font-medium text-sm">All synced</span>
      <span className="text-xs opacity-75">{formatLastSync(status.lastSyncTime)}</span>
    </div>
  );
};
